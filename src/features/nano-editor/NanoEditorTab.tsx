...
export default function NanoEditorTab() {
  ...
  // ── Mode: result exists → iterative view ─────────────────────────────────
    ...
    return (
      <div className="space-y-4">
        <section className="panel-surface space-y-4">
          ...
          {/* Output image */}
          <div className="relative flex items-center justify-center overflow-hidden rounded-2xl border border-slate-700/70 bg-slate-900/45">
            <AnimatePresence mode="wait">
              {isGenerating ? (
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="flex flex-col items-center py-24 text-slate-300"
                >
          ...
          {!isGenerating && (
            <div className="space-y-3">
              <textarea
                ...
                rows={3}
                className="w-full rounded-2xl border border-slate-700/80 bg-slate-900/70 p-4 text-sm text-slate-100 outline-none transition-colors placeholder:text-slate-500 focus:border-cyan-300/70"
                onKeyDown={(e) => {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <section className="panel-surface flex items-center justify-center">
